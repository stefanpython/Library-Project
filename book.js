...
addBookBtn.addEventListener("click", (event) => {
  if (myLibrary.length != 0) {
    const bookContainer = document.querySelector(".bookCard");
...
function displayBooks() {
  ...
  myLibrary.forEach((book, index) => {
    ...
    div.innerHTML = `
      <div><h3>Title: ${book.title}</h3></div>
      <hr id='cardLines'>
      <div><h3>Author: ${book.author}</h3></div>
      <hr id='cardLines'>
      <div><h3>Pages: ${book.pages}</h3></div>
      <hr id='cardLines'>
      <div><h3>Read: <button class='readBtn' id='${index}'>${book.read}</button></h3></div>
...
function showForm() {
  ...
  if (myLibrary.length !== 0) {
    const bookContainer = document.querySelectorAll(".bookCard");
